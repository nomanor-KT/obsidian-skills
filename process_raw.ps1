...
function Detect-Type {
    ...
    # Priority: kontakt > spotkanie > projekt > surcharge > newsletter/intel > operacja > notatka
    if ($Text -match '(?i)\bphone\b|\bkontakt\b|\bvisiting card\b|\bwizytowka\b|\bmobile\b') { return 'kontakt' }
    ...
    if ($Text -match '(?i)\binstruction\b|\binstrukcj\w*\b|\boperacj\w*\b') { return 'operacja' }
    return 'notatka'
}
...
function Extract-Hashtags {
    ...
    foreach ($line in $Lines) {
        ...
        foreach ($match in [regex]::Matches($line, '(?<!\S)#([A-Za-z0-9_-]+)')) {
            $results.Add($match.Groups[1].Value.ToLowerInvariant())
        }
    }
...
function Get-SubjectLine {
    ...
    # If subject starts with markdown header marks, strip them
...
function Test-ReadyForDelete {
    ...
    if ($TargetFileName -notmatch '^20\d{2}-\d{2}-\d{2}-[a-z0-9-]+\.md$') { return $false }

    $saved = Get-Content -LiteralPath $TargetPath -Raw
    if ($saved -notmatch '(?ms)^---\s*\r?\n.*?\btyp:\s*.+\r?\n.*?\bdate:\s*\d{4}-\d{2}-\d{2}\r?\n.*?\bsource:\s*RAW\r?\n.*?\btags:\s*\[.+\]\r?\n.*?\bstatus:\s*(aktywny|zamkniety|czekam)\r?\n---') { return $false }

    if ($saved -notmatch '(?m)^##\s+Kluczowe punkty\s*$') { return $false }
    if ($saved -notmatch '(?m)^##\s+Wymagane dzialania\s*$') { return $false }
    if ($saved -notmatch '(?m)^##\s+Powiazane notatki\s*$') { return $false }

    if ($RelatedLinks.Count -lt 1) { return $false }
    if ($saved -notmatch [regex]::Escape($ExpectedMoc)) { return $false }

    return $true
}

$rawPath = "Obsidian/Karol-KB/RAW"
$baseDir = "Obsidian/Karol-KB"
$files = Get-ChildItem -LiteralPath $rawPath -File -ErrorAction SilentlyContinue | Where-Object { $_.Extension -in '.txt', '.md' }
$results = @()
...
foreach ($file in $files) {
    try {
        ...
        $sourceDate = Get-DateFromText -FullText $fullText -FileName $file.Name
        ...
        $subject = Get-SubjectLine -Lines $cleanLines.ToArray() -Fallback $file.BaseName
        ...
        $hashtags = Extract-Hashtags -Lines $cleanLines.ToArray()
...
## Aktualizacja z RAW ($($file.Name))
$($bullets -join "`n")
"@
            Add-Content -LiteralPath $targetPath -Value $append
        } else {
            Set-Content -LiteralPath $targetPath -Value $noteContent
        }

        $safeToDelete = Test-ReadyForDelete -TargetPath $targetPath -TargetFileName $targetFileName -BodyText $noteContent -RelatedLinks $relatedLinks -ExpectedMoc $expectedMoc
        if ($safeToDelete) {
        ...
        Write-Warning "Failed processing $($file.Name): $($_.Exception.Message)"
    }
}
